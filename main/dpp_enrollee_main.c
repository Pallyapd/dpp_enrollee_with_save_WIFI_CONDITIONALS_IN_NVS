...
void dpp_enrollee_init(void)
{
    ...
    if (bits & DPP_CONNECTED_BIT) {
        ...
        ESP_ERROR_CHECK(ret);
        ret = nvs_set_str(nvs_handle, "wifi_ssid", & (s_dpp_wifi_config.sta.ssid));
        ESP_ERROR_CHECK(ret);
        ret = nvs_set_str(nvs_handle, "wifi_password", &s_dpp_wifi_config.sta.password);
        ESP_ERROR_CHECK(ret);
...
void get_ssid_pas()
{
 ...
 // ESP_ERROR_CHECK(esp_wifi_start());
    ...
    esp_err_t err = nvs_open("storage", NVS_READONLY, &nvs_handle);
        ...
        ESP_LOGI(TAG, "ssid in memcpy:%s", wifi_config.sta.ssid);
    ...
    if (ssid_size > 0 && password_size > 0) {
        printf("Wi-Fi credentials are saved in NVS\n");
    } else {
        printf("Wi-Fi credentials are not found in NVS\n");
    }


}
void wifi_ssid_pass_check(void)
{
     nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open("storage", NVS_READONLY, &nvs_handle);
        if (err != ESP_OK) {
        ...
        return;
         ...
         // Check if Wi-Fi SSID exists in NVS
        size_t ssid_size = 0;
        err = nvs_get_str(nvs_handle, "wifi_ssid", NULL, &ssid_size);

        if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
            // Handle error
        printf("Failed to read Wi-Fi SSID from NVS\n");
        nvs_close(nvs_handle);
        return;
        }
        size_t password_size = 0;
        ...
        if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
              // Handle error
        printf("Failed to read Wi-Fi password from NVS\n");
        nvs_close(nvs_handle);
        return;
        }
        nvs_close(nvs_handle);
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(ret);
    wifi_ssid_pass_check();
    
    if(Wifi_set_bit == true){
